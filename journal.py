...
def main():
	...
	group.add_argument("-A",  "--Archive",  dest="action",  action="store_const",  const="archive",  help="archive to datetimed tarball")
	group.add_argument("-C",  "--Count",    dest="action",  action="store_const",  const="count",    help="count words and entries")
	group.add_argument("-G",  "--Graph",    dest="action",  action="store_const",  const="graph",    help="graph entry references in DOT")
	group.add_argument("-L",  "--List",     dest="action",  action="store_const",  const="list",     help="list entry dates")
	group.add_argument("-S",  "--Show",     dest="action",  action="store_const",  const="show",     help="show entries")
	group.add_argument("-T",  "--Tag",      dest="action",  action="store_const",  const="tag",      help="create tags file")
	group.add_argument("-V",  "--Verify",   dest="action",  action="store_const",  const="verify",   help="verify journal sanity")
	group = arg_parser.add_argument_group("OUTPUT OPTIONS")
	group.add_argument("-d",  "--date-range",      dest="date_range",   action="store",        help="only use entries in range")
	group.add_argument("-i",  "--no-ignore-case",  dest="ignore_case",  action="store_false",  help="ignore ignore case")
	group.add_argument("-n",  "--num-results",     dest="num_results",  action="store",        help="max number of results")
	group.add_argument("-r",  "--reverse",         dest="reverse",      action="store_true",   help="reverse chronological order")
